...
import { createNewUser } from '~/lib/server/user/create-user';
...
export const createNewUserAction = withAdminSession(
  async ({ userData }: { userData: NewUserData; csrfToken: string }) => {
    const logger = getLogger();

    logger.info('Admin requested to create a new user account');
    ...
    revalidatePath('/admin/create-new-user', 'page');
    
    logger.info('New user created');
  
    redirect('/admin/create-new-user');
  },
);
